...
export const TargetCursor = ({
    ...
    // `padding` is the outward gap between the bracket and the text/element
    // edge — kept separate from `borderWidth` (the visual stroke thickness)
    // so the bracket can sit a bit further out without a thicker line.
    ...
    // Scale the bracket to the hovered element's font size, so a small nav
    // link gets a tight bracket and a large display heading gets a bracket
    // that still hugs its text instead of floating a fixed 12px away.
    const activeMetricsRef = useRef(constants)

    const getCornerMetrics = useCallback(
        (target: Element) => {
            // The visible text is often on a nested element (e.g. an <h3>
            // sized via inline style) rather than the matched .cursor-target
            // itself, so scan descendants for the largest rendered font size.
            let fontSize = 0
            for (const el of [target, ...target.querySelectorAll('*')]) {
                const fs = parseFloat(getComputedStyle(el).fontSize)
                if (Number.isFinite(fs) && fs > fontSize) fontSize = fs
            }
            if (fontSize <= 0) return constants
            const cornerSize = Math.min(28, Math.max(10, fontSize * 0.4))
            const borderWidth = Math.min(5, Math.max(2, cornerSize * 0.22))
            const padding = Math.min(14, Math.max(6, cornerSize * 0.45))
            return { cornerSize, borderWidth, padding }
        },
        [constants]
    )
        ...
        const enterHandler = (e: MouseEvent) => {
            ...
            const leaveHandler = () => {
                ...
                if (cornersRef.current) {
                    ...
                    activeMetricsRef.current = constants
                    gsap.to(corners, {
                        width: cornerSize,
                        height: cornerSize,
                        borderWidth,
